module Flexite
  class Diff
    class ApplyService
      def initialize(ids)
        @ids = ids
        @diffs = ids.present? ? Diff.find(ids) : {}
        @grouped_diff = @diffs.group_by(&:change_type)
        @objects_to_save = []
      end
      ...
      def call
        Config.transaction(requires_new: true) do
          Entry.transaction do
            %w[~ + -].each do |type|
              next if @grouped_diff[type].blank?

              @grouped_diff[type].map do |diff|
                send("handle_#{diff.change_type}", diff.path, *diff.hash_changes)
              end
              @objects_to_save.each(&:save) if type == '+'

              Flexite::Config.where(to_delete: true).destroy_all
              Flexite::Entry.where(to_delete: true).destroy_all
              Flexite::Diff.destroy(@ids)
            end
          end
        end
      ...
      define_method('handle_+') do |depth, record|
        object = dig(Config.roots.order_by_name, depth.slice(0..-2))
        case object
      ...
      define_method('handle_-') do |depth, _|
        record = dig(Config.roots.order_by_name, depth)
        record.update_column(:to_delete, true)
      end

      define_method('handle_~') do |depth, new_value, _|
        attr_name = depth.pop
        record = dig(Config.roots.order_by_name, depth)
